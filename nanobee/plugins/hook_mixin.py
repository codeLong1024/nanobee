...
class PluginHookMixin:
    ...
    插件可以混入此类（可选），在 Agent 生命周期的 5 个切面注入逻辑。

    使用示例::

        class MyMemoryPlugin(PluginHookMixin, MemoryPlugin):
    ...
    注意：``NanobeePlugin`` 基类已包含这 5 个方法的默认空实现，
    即使不混入此类，插件也可以直接覆盖对应方法。
    ``PluginHookMixin`` 仅作为类型提示和文档引用。
    """
